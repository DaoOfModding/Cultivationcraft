...
import DaoOfModding.Cultivationcraft.Common.Capabilities.PlantGenerator.PlantGeneratorCapability;
...
public class CapabilityListeners {
    ...
    public static final ResourceLocation PlantGeneratorCapabilityLocation = new ResourceLocation(Cultivationcraft.MODID, "plantgenerator");

    @SubscribeEvent
    public static void attachCapabilitiesLevel(final AttachCapabilitiesEvent<Level> event) {
        if (!event.getCapabilities().containsKey(PlantGeneratorCapabilityLocation)) {
            event.addCapability(PlantGeneratorCapabilityLocation, new PlantGeneratorCapability());
        }
    }
